"""Task: live differential fuzz — py4dggs.RTEA7H vs the pydggal RTEA7H_Z7 oracle.

Verification model (mirrors test_ivea7h_fuzz.py, established 2026-07-01):
  - py4dggs geometry agrees with pydggal to ~1e-12 deg but is NOT bit-identical
    ...
    match exactly.
  - The RTEA projection's correctness is PROVEN by centroid + round-trip agreeing
    ...
    class as ISEA's/IVEA's documented singularities), never a projection error --
    so the forward test allows a small tie rate rather than demanding 100%.
  - A known pre-existing bug (in the frozen igeo7-py too) gives a small fraction
    of cells a degenerate (0,0) vertex; those cells' vertices/neighbours are
    skipped here, mirroring the IVEA7H treatment.
"""
import math
import random
...
@requires_pydggal
def test_rtea7h_centroid_matches_pydggal():
    """The strong correctness gate: cell centroid within TOL of pydggal."""
    g = oracle_grid("RTEA7H_Z7")
...
@requires_pydggal
def test_rtea7h_projection_roundtrips():
    """forward->inverse must return the input point (self-consistency)."""
    geom = RTEA7H._geom; proj = RTEA7H.projection
    rng = random.Random(20260701)
    for _ in range(500):
...
@requires_pydggal
def test_rtea7h_forward_matches_pydggal():
    ...
    for _ in range(1000):
        ...
        got = RTEA7H.zone_from_geo(lat, lon, res).text_id
        n += 1; ok += (got == want)
    assert ok / n >= 0.99, f"RTEA forward match rate {ok}/{n} below 99% (projection bug, not a tie)"


@requires_pydggal
def test_rtea7h_vertices_match_pydggal():
    """Vertices match pydggal as an unordered set within TOL, skipping degenerate
    (0,0)-vertex cells (pre-existing shared bug, same treatment as IVEA7H)."""
    g = oracle_grid("RTEA7H_Z7")
    rng = random.Random(20260701)
    checked = skipped = 0
    for _ in range(500):
...
@requires_pydggal
def test_rtea7h_neighbours_match_pydggal():
