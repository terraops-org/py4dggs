"""Live differential fuzz: py4dggs.IVEA3H vs pydggal IVEA3H (point-keyed geometry).
...
import random
import pytest
from py4dggs import IVEA3H, Zone
...
def test_fuzz_geometry_matches_pydggal():
    g = oracle_grid("IVEA3H"); rng = random.Random(20260703)
    ...
    for _ in range(500):
        ...
        z = Zone(IVEA3H, val)                    # DGGAL-layout int == our value
        # centroid
...
def test_neighbours_exact_any_seed():
    ...
    g = oracle_grid("IVEA3H"); rng = random.Random(88881)
    ...
    for _ in range(1500):
        ...
        if onb is None:
            continue
        nb_checked += 1
        if {n.value for n in Zone(IVEA3H, val).neighbors} != onb:
            n_fail += 1
